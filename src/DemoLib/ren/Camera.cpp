...
void Ren::Camera::Move(const Vec3f &v, float delta_time) {
    ...
    // view_matrix_[2]*view_matrix_[14]); world_position_[1] = -(view_matrix_[4]*view_matrix_[12] +
    // view_matrix_[5]*view_matrix_[13] + view_matrix_[6]*view_matrix_[14]); world_position_[2] =
    // -(view_matrix_[8]*view_matrix_[12] + view_matrix_[9]*view_matrix_[13] + view_matrix_[10]*view_matrix_[14]);
}
...
void Ren::Camera::Rotate(float rx, float ry, float delta_time) {
    ...
    rot_matrix =
        Ren::Rotate(rot_matrix, rx * delta_time, Vec3f{view_matrix_[0][0], view_matrix_[1][0], view_matrix_[2][0]});
    rot_matrix =
        Ren::Rotate(rot_matrix, ry * delta_time, Vec3f{view_matrix_[0][1], view_matrix_[1][1], view_matrix_[2][1]});
...
void Ren::Camera::UpdatePlanes() {
    ...
    for (int pl = LeftPlane; pl <= FarPlane; pl++) {
        float inv_l = 1.0f / std::sqrt(frustum_.planes[pl].n[0] * frustum_.planes[pl].n[0] +
...
Ren::eVisibilityResult Ren::Camera::CheckFrustumVisibility(const Vec3f &bbox_min, const Vec3f &bbox_max) const {
    const float bbox_points[8][3] = {{bbox_min[0], bbox_min[1], bbox_min[2]}, {bbox_max[0], bbox_min[1], bbox_min[2]},
...
float Ren::Camera::GetBoundingSphere(Vec3f &out_center) const {
    ...
    float k =
        std::sqrt(1 + (1.0f / (aspect_ * aspect_))) * aspect_ * std::tan(0.5f * angle_ * Ren::Pi<float>() / 180.0f);
    float k_sqr = k * k;
    if (k_sqr >= (f - n) / (f + n)) {
...
void Ren::Camera::ExtractSubFrustums(int resx, int resy, int resz, Frustum *sub_frustums) const {
    ...
    { // Construct cells for the first depth slice
        const float znear = near_, zfar = near_ * std::pow(far_ / near_, 1.0f / resz);
        ...
        for (int y = 0; y < resy; y++) {
            ...
            for (int x = 0; x < resx; x++) {
                ...
                const Ren::Vec3f _p0 = Ren::Vec3f{p0 / p0[3]}, _p1 = Ren::Vec3f{p1 / p1[3]},
                                 _p2 = Ren::Vec3f{p2 / p2[3]}, _p3 = Ren::Vec3f{p3 / p3[3]};
