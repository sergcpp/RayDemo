...
sw_inline void _swProcessPixelLine(SWprogram *p, SWframebuffer *f, SWint x, SWint y,
                                   SWint b_depth_test, SWint b_depth_write, SWfloat *vs_out_res) {
    SWfloat f_out[4];
    SWint b_discard = 0;

    if (b_depth_test && !swFbufTestDepth(f, x, y, vs_out_res[2])) return;
...
void _swProcessCurveRecursive(SWprogram *p, SWframebuffer *f,
                              SWfloat p1[SW_MAX_VTX_ATTRIBS], SWfloat p2[SW_MAX_VTX_ATTRIBS],
                              SWfloat p3[SW_MAX_VTX_ATTRIBS], SWfloat p4[SW_MAX_VTX_ATTRIBS],
                              SWint b_depth_test, SWint b_depth_write, SWfloat tolerance);

sw_inline void _swProcessCurve(SWprogram *p, SWframebuffer *f, SWfloat vs_out[4][SW_MAX_VTX_ATTRIBS],
                               SWint _1, SWint _2, SWint _3, SWint _4, SWint b_depth_test, SWint b_depth_write, SWfloat tolerance) {
    _swProcessCurveRecursive(p, f, vs_out[_1], vs_out[_2], vs_out[_3], vs_out[_4], b_depth_test, b_depth_write, tolerance);
}
...
sw_inline void _swProcessPixel_correct(SWprogram *p, SWframebuffer *f, SWint ix, SWint iy,
                                       SWfloat uvw[3], SWfloat *f0, SWfloat *f1, SWfloat *f2, SWfloat *vs_interpolated,
                                       SWint b_depth_test, SWint b_depth_write, SWint b_blend) {
    SWfloat depth;
    if (_swIntepolateTri_correct(p, f, b_depth_test, &depth, ix, iy, f0, f1, f2, uvw, vs_interpolated)) {
        SWfloat f_out[4];
        SWint b_discard = 0;
...
sw_inline void _swProcessTriangle_correct(SWprogram *p, SWframebuffer *f, SWfloat vs_out[3][SW_MAX_VTX_ATTRIBS], SWint _0, SWint _1,
        SWint _2, SWint b_depth_test, SWint b_depth_write, SWint b_blend) {
    ...
    if (min[0] >= f->w || min[1] >= f->h || max[0] < 0 || max[1] < 0) return;

    if (min[0] < 0) min[0] = 0;
    if (min[1] < 0) min[1] = 0;
    if (max[0] >= f->w) max[0] = f->w - 1;
    if (max[1] >= f->h) max[1] = f->h - 1;
    ...
    for (y = min[1]; y < max[1]; y += SW_TILE_SIZE) {
        for (x = min[0]; x < max[0]; x += SW_TILE_SIZE) {
            SWint x0 = x,
                  x1 = sw_min(x + SW_TILE_SIZE - 1, f->w - 1),
                  y0 = y,
            ...
            if (a == 15 && b == 15 && c == 15 && full_tile) {
                ...
                for (iy = y; iy < y + SW_TILE_SIZE; iy++) {
                    ...
                    for (ix = x; ix < x + SW_TILE_SIZE; ix++) {
                        ...
                        vs_interpolated[2] = uvw[0] * pos0[2] + uvw[1] * pos1[2] + uvw[2] * pos2[2];

                        SWfloat inv_w = 1 / (uvw[0] * pos0[3] + uvw[1] * pos1[3] + uvw[2] * pos2[3]);
                        uvw[0] *= inv_w;
                        uvw[1] *= inv_w;
                        uvw[2] *= inv_w;

                        _swProcessPixel_correct(p, f, ix, iy, uvw, pos0, pos1, pos2, vs_interpolated,
                                                b_depth_test, b_depth_write, b_blend);
...
sw_inline void _swProcessPixel_nocorrect(SWprogram *p, SWframebuffer *f, SWint ix, SWint iy,
        SWfloat vs_interpolated[SW_MAX_VTX_ATTRIBS],
        SWint b_depth_test, SWint b_depth_write, SWint b_blend) {
    SWfloat depth;
...
sw_inline void _swProcessTriangle_nocorrect(SWprogram *p, SWframebuffer *f, SWfloat vs_out[3][SW_MAX_VTX_ATTRIBS], SWint _0, SWint _1,
        SWint _2, SWint b_depth_test, SWint b_depth_write, SWint b_blend) {
    ...
    for (y = min[1]; y < max[1]; y += SW_TILE_SIZE) {
        for (x = min[0]; x < max[0]; x += SW_TILE_SIZE) {
            ...
            SWfloat f00_z = pos0[2] + Cy3 * inv_area * vs_d10[2] + Cy1 * inv_area * vs_d20[2];
            SWfloat f10_z = pos0[2] + Cy3_10 * inv_area * vs_d10[2] + Cy1_10 * inv_area * vs_d20[2];
            SWfloat f01_z = pos0[2] + Cy3_01 * inv_area * vs_d10[2] + Cy1_01 * inv_area * vs_d20[2];
            SWfloat f11_z = pos0[2] + Cy3_11 * inv_area * vs_d10[2] + Cy1_11 * inv_area * vs_d20[2];
            ...
            for (i = 0; i < num_attributes; i++) {
                ...
                vs_interpolated[i] = pos0[i] + Cy3 * inv_area * vs_d10[i] + Cy1 * inv_area * vs_d20[i];
            }
...
sw_inline void _swProcessTriangle_fast(SWprogram *p, SWframebuffer *f, SWfloat vs_out[3][SW_MAX_VTX_ATTRIBS], SWint _0, SWint _1,
                                       SWint _2, SWint b_depth_test, SWint b_depth_write, SWint b_blend) {
    SWint i, x, y, p0[2], p1[2], p2[2];
    SWint min[2], max[2];
    SWfloat *pos0 = vs_out[_0],
    ...
    for (y = min[1]; y < max[1]; y += SW_TILE_SIZE) {
        for (x = min[0]; x < max[0]; x += SW_TILE_SIZE) {
            ...
            SWint a = ((Cy1_00 > 0) << 0) | ((Cy1_10 > 0) << 1) | ((Cy1_01 > 0) << 2) | ((Cy1_11 > 0) << 3);
            SWint b = ((Cy2_00 > 0) << 0) | ((Cy2_10 > 0) << 1) | ((Cy2_01 > 0) << 2) | ((Cy2_11 > 0) << 3);
            SWint c = ((Cy3_00 > 0) << 0) | ((Cy3_10 > 0) << 1) | ((Cy3_01 > 0) << 2) | ((Cy3_11 > 0) << 3);
            ...
            SWfloat f00[SW_MAX_VTX_ATTRIBS],
                    f10[SW_MAX_VTX_ATTRIBS],
                    f01[SW_MAX_VTX_ATTRIBS],
                    f11[SW_MAX_VTX_ATTRIBS];
            ...
            f00[2] = pos0[2] + Cy3_00 * inv_area * vs_d10[2] + Cy1_00 * inv_area * vs_d20[2];
            f10[2] = pos0[2] + Cy3_10 * inv_area * vs_d10[2] + Cy1_10 * inv_area * vs_d20[2];
            f01[2] = pos0[2] + Cy3_01 * inv_area * vs_d10[2] + Cy1_01 * inv_area * vs_d20[2];
            f11[2] = pos0[2] + Cy3_11 * inv_area * vs_d10[2] + Cy1_11 * inv_area * vs_d20[2];
            ...
            if (full_cover) {
                ...
                for (iy = y; iy < y + SW_TILE_SIZE; iy++) {
                    ...
                    for (ix = x; ix < x + SW_TILE_SIZE; ix++) {
                        _swProcessPixel_fast(p, f, ix, iy, vs_interpolated, depth_test, b_depth_write, b_blend);
...
sw_inline SWint _swClipAndTransformToNDC_Line(SWfloat vs_in[2][SW_MAX_VTX_ATTRIBS], SWfloat vs_out[2][SW_MAX_VTX_ATTRIBS], const SWint num_attrs, const SWint num_corr_attrs) {
    ...
    for (i = 0; i < 2; i++) {
        for (j = 0; j < 6; j++) {
            if (PLANE_DOT(vs_in[i], clip_planes[j]) < 0) clip_mask |= clip_plane_bits[j];
        }
    }
