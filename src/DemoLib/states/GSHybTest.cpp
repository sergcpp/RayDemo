...
void GSHybTest::Enter() {
    ...
    // TODO: fix this
    cpu_scene_->GetCamera(Ray::CameraHandle{0}, cam_desc);
...
void GSHybTest::Draw(uint64_t dt_us) {
    ...
    { // update camera
        ...
        for (auto &s : gpu_scenes_) {
            s->SetCamera(Ray::CameraHandle{0}, cam_desc);
        }
        cpu_scene_->SetCamera(Ray::CameraHandle{0}, cam_desc);
    }
...
void GSHybTest::Update(uint64_t dt_us) {
    ...
    if (animate_) {
        ...
        cpu_scene_->SetMeshInstanceTransform(Ray::MeshInstanceHandle{1}, ValuePtr(tr));
    }
